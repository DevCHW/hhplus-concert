...
class QueueService(
     ...
     * 토큰 생성
     */
    @DistributedLock(resource = LockResource.TOKEN, key = "#userId", strategy = LockStrategy.REDIS_PUB_SUB)
    fun createQueueToken(userId: String): Token {
        if (queueRepository.isExistByUserId(userId)) {
    ...
    fun getToken(token: UUID): Token {
        return queueRepository.getByToken(token)
    }
    ...
    fun expireActiveTokens(activeTokenLifeTime: Long, now: LocalDateTime) {
        val activeTokens = queueRepository.getTokenByStatus(Token.Status.ACTIVE)
        ...
        if (expiredActiveTokens.isNotEmpty()) {
            ...
            queueRepository.deleteByIds(expiredTokenIds)
        }
    }
    ...
    fun activateTokens(activeTokenMaxSize: Int) {
        ...
        val inactiveTokens = queueRepository.getTokenByStatusNotSortByIdAsc(Token.Status.ACTIVE, targetCount)
        ...
        val result = queueRepository.updateStatusByIdsIn(Token.Status.ACTIVE, inactiveTokenIds)
    }
